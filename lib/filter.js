...
var Filter = function (setting) {
    ...
    filter.checkAscii = function (problem, index) {
        if (filter.setting.isAsciiOnly(index)) {
            if (problem.title.split('').some(function (c) {
                    return c > String.fromCharCode(127);
                })) {
                return false;
            }
    ...
    filter.hasNewTag = function (problem, tags) {
        return problem.tags.some(function (tag) {
            return !tags.hasOwnProperty(tag);
        });
    };

    filter.addToTag = function (problem, tags) {
            ...
            util.newIncArray(set_num).forEach(function () {
                ...
                problem_inc_array.forEach(function (index) {
                    ...
                    while (filtered_index[index] < filtered_problems[index].length) {
                        problem = filtered_problems[index][filtered_index[index]];
                        key = problem.id;
                        filtered_index[index] += 1;
                        if (selected_sub.indexOf(key) === -1 && !selected_total.hasOwnProperty(key)) {
                            if (setting.isForceNewTag()) {
                                if (filter.hasNewTag(problem, appeared_tags)) {
                                    selected_sub[index] = key;
                                    appeared_tags = filter.addToTag(problem, appeared_tags);
                                    break;
                                }
                            } else {
                                selected_sub[index] = key;
                ...
                if (setting.isRandomOrder()) {
                    selected_sub.forEach(function (sub, idx) {
                        var randIdx = Math.floor(Math.random() * idx),
                            temp = sub;
                        selected_sub[idx] = selected_sub[randIdx];
                        selected_sub[randIdx] = temp;
                    });
                }
...
exports.outputFilteredProblemSets = function (setting, callback) {
    exports.getFilteredProblemSets(setting, function (problem_sets, team_accepts, chase_accepts, problems) {
        var isShowTitle = setting.isShowProblemTitle(),
            ...
            isShowUrl = setting.isShowProblemUrl(),
            plain = [],
            detail = [];
        problem_sets.forEach(function (problem_set, index) {
            ...
            problem_set.forEach(function (problem) {
                ...
                if (isShowUrl) {
                    detail.push(chalk.gray('http://codeforces.com/problemset/problem/' + problem.num + '/' + problem.alpha));
                }
            });
            plain.push('');
            detail.push('');

...
var ProblemCrawler = function (setting) {
    ...
    crawler.save = function () {
        var mkdirp = require('mkdirp'),
            temp_path = PROBLEM_FILE + '.tmp';
        mkdirp(util.getSaveDirectory(), function (err) {
            if (err) {
                console.log(err);
                return;
            }
            fs.writeFile(temp_path, JSON.stringify(crawler.problems), function (err) {
                if (err) {
                    console.log(err);
                    return;
                }
                fs.rename(temp_path, PROBLEM_FILE, function (err) {
                    if (err) {
                        console.log(err);
                    }
                });

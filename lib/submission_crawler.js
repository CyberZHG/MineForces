...
var SubmissionCrawler = function (setting, user_id) {
    ...
    crawler.save = function () {
        var temp_path = crawler.getSaveFilePath() + '.tmp';
        crawler.user_info.max_sub_num = crawler.max_sub_num;
        fs.writeFile(temp_path, JSON.stringify(crawler.user_info), function (err) {
            if (err) {
                console.log(err);
            } else {
                fs.rename(temp_path, crawler.getSaveFilePath(), function (err) {
                    if (err) {
                        console.log(err);
                    }
                });
        ...
        fs.readFile(crawler.getSaveFilePath(), function (err, data) {
            if (err) {
                ...
                try {
                    ...
                    if (!crawler.setting.isSilent()) {
                        log.fail('Error while reading submissions: ' + e);
                    }
                }
                if (callback) {
                    callback();
    ...
    crawler.pullSubmissions = function (callback) {
        fs.stat(crawler.getSaveFilePath(), function (err, stats) {
            if (err) {
                ...
                if (diff > 1000 * 60 * 10
                        || Object.keys(crawler.user_info.accepts).length === 0
                        || crawler.setting.isForceUpdate()) {
                    crawler.pullSubmissionsAt(1, 0, callback);
                } else {
                    if (callback) {
                        callback(crawler.user_info);
